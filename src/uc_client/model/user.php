...
class usermodel
{
    ...
    function check_username($username)
    {
        ...
        if ($len > 35 || $len < 3 || preg_match("/\s+|^c:\\con\\con|[%,\*\"\s\<\>\&]|$guestexp/is", $username)) {
            return FALSE;
        } else {
            return TRUE;
        }
	...
	$this->db->query("INSERT INTO ".UC_DBTABLEPRE."members SET $sqladd username='$username', password='$password', email='$email', role='$role', domain_user='$domain_user',meta='$meta',regip='$regip', regdate='".$this->base->time."', salt='$salt'");
