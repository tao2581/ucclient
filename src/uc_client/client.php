...
defined('IN_UC') or define('IN_UC', TRUE);
defined('UC_CLIENT_VERSION') or define('UC_CLIENT_VERSION', '1.6.0');
defined('UC_CLIENT_RELEASE') or define('UC_CLIENT_RELEASE', '20110501');
defined('UC_ROOT') or define('UC_ROOT', substr(__FILE__, 0, -10));
defined('UC_DATADIR') or define('UC_DATADIR', UC_ROOT . './data/');
defined('UC_DATAURL') or define('UC_DATAURL', UC_API . '/data');
defined('UC_API_FUNC') or define('UC_API_FUNC', UC_CONNECT == 'mysql' ? 'uc_api_mysql' : 'uc_api_post');
...
function uc_user_register($username, $password, $email, $role, $domain_user,$meta = '', $questionid = '', $answer = '', $regip = '') {
	return call_user_func(UC_API_FUNC, 'user', 'register', array('username'=>$username, 'password'=>$password, 'email'=>$email, 'role'=>$role,'questionid'=>$questionid, 'answer'=>$answer, 'regip' => $regip,'domain_user'=>$domain_user,'meta'=>$meta));
}
